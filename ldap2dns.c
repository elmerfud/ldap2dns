...
static void write_rr(struct resourcerecord* rr, int ipdx, int znix)
{
	...
	if (strcasecmp(rr->type, "NS")==0) {
		...
		if (tinyfile) {
			/* string substitutions for tinydns data file */
			if(rr->cname!=NULL) {
				char *spftmp = NULL;
				if(strstr(rr->cname,":") != NULL) {
					spftmp = calloc(1024, sizeof(char));
					spftmp = replace(rr->cname,":","\\072");
					sprintf(rr->cname, "%s", spftmp);
					free(spftmp); spftmp=NULL;
				}
				if(strstr(rr->cname,"+") != NULL) {
					spftmp = calloc(1024, sizeof(char));
					spftmp = replace(rr->cname,"+","\\053");
					sprintf(rr->cname, "%s", spftmp);
					free(spftmp); spftmp=NULL;
				}
			}
